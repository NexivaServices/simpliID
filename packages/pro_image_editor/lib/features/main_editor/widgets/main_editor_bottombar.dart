...
class MainEditorBottombar extends StatelessWidget {
  ...
  /// - [openRemoveBackground]: Callback for removing background.
  const MainEditorBottombar({
    ...
    required this.openRemoveBackground,
  });
  ...
  /// Callback for removing background.
  final Function() openRemoveBackground;
    ...
    return configs.mainEditor.tools
        .map((tool) {
          switch (tool) {
            ...
            case SubEditorMode.removeBackground:
              return _buildActionButton(
                key: const ValueKey('remove-background-btn'),
                label: 'Remove BG',
                icon: Icons.auto_fix_high,
                onPressed: openRemoveBackground,
              );
          }
        })
        .whereType<Widget>()
